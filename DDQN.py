...
path = f'./results_DDQN/{now_day}_{X_SIZE, Y_SIZE}_{GAMMA}_NE_{NUM_EPISODES}_TSL_{TIME_LIMIT}_{TARGET_UPDATE}_{RANDOM_ACTION_PROB}_{RM_SIZE}/result_{now}'
    ...
    for i_episode in range(1, NUM_EPISODES+1):
        ...
        for t in range(1, TIME_LIMIT+1): # t = 1 ~ TIME_LIMIT
            ...
            if steps_done % TARGET_UPDATE == 0:
                target_Q_net.load_state_dict(Q_net.state_dict())    # copy q() to q_target()
            if done:
                break
